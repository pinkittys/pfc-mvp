...
async def final_recommend(req: UnifiedRecommendRequest):
    ...
    try:
        ...
        print(f"   요청 색상( selected_keywords.colors ): {req.selected_keywords.colors if req.selected_keywords else None}")

        # 5) 스프레드시트 매칭 시도 → 실패 시 기존 매처
        ...
        # 7) 추천 이유/카드/시즌
        from app.api.v1.endpoints.recommend import _generate_unified_recommendation_reason, _generate_flower_card_message, _get_season_info
        ...
        return {
            ...
            "calligraphy_image_url": matched_flower.image_url,  # NOTE: 기존 로직 유지
            "flower_card_message": {
